...
Warning: depreciated, Goldencoin v1.5.x no longer support GNUroot App
Warning: alpha release, suitable for expert linux developers only

This folder describes how to set up full node of Goldencoin (GDNC) on 32 bits android phone on arm CPUs. 
...
https://github.com/ShorelineCrypto/GoldenCoin/releases/download/GDNC_2020_Q3_report/GDNC_2020_Q3_report.pdf 
...
### Step 9: install some tools, prepare GDNC required library

 screen is useful tools to run in background.  busybox top or uptime is better tool for 
than the default ones.  "busybox top" can see GDNC mining command and android processes.  
...
## From Desktop Linux in Virtualbox
### Step 10: Compile GDNC wallet in QEMU
  The armel/armhf GDNC binary from our download is unlikely to work well if your phone hardware is different. 
  Explained in pdf report, you should compile Berkley DB libary above successfully, but likely to fail on GDNC core compilation.  Extract and copy DB library files into QEMU and compile GDNC core wallet following cloud boot from below guide:
 ...
 After successfully compiling the GDNC wallet files, copy the files into the android phone for use. 
...
```
   ...
   GDNC full node can run in one screen session, the cheetah runs in another screen session. 
