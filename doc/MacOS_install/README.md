...
For full guide of building own wallet from source, please follow steps at:
 https://github.com/ShorelineCrypto/GoldenCoin/blob/master/doc/build-osx.md

Here below we describe an easier way to install macOS binary wallet on GDNC v1.7.0.0, which was built from source in Mojave. The below 
...
Brew now installed newer and imcompatible versions.  Older version of libraries are enabled as default in order to run GDNC.
...
Enable or not, GDNC GUI and CLI wallet was tested to be working in MacOS Catalina. 
...
For first time GDNC user only to create data folder. If you have never ran GDNC wallet before,  click QT wallet file first, run for a 1 minute, then shut down the wallet.

Copy the conf file to your Goldencoin data directory by running below at terminal:
...
When you are running GDNC already or after running, you may need to restore system (newer) versions on openssl/miniupnpc to allow 
