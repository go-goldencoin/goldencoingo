...
This folder describe how to run a full node of Goldencoin (GDNC) and to CPU mine GDNC in arm based Chromebook as Linux App.  Chrome OS on arm64 or armhf platform was tested. Any Chrome OS version v69 or later on 64 bits arm64 or 32 bits armhf  with linux either in linux beta (Crostini) or Android UserLand app should all work. 
...
Go to Chrome OS setting, turn on Linux (Beta).  If your chromebook does not have this option, it is not supported here for GDNC CPU mining. However, your arm CPU based chromebook may still work on Android UserLand app method as alternative. 
...
```
   tar xvfz goldencoin_v1.7.0.3_chromeos_arm.tgz
...
After successfully completing above commands, GDNC wallet file and Cheetah_Cpuminer will be downloaded at your current folder. You can move these files to whatever best location inside your "Linux files" folder by using either linux command line or Chromebook GUI drag and drop. 
...
For GDNC cpu mining purpose, either CLI wallet or GUI QT wallet was tested working in Chromebook.  You can run either of the software
...
### arm64
```
 hlu@penguin:~$ cd  goldencoin_v1.7.0.2_u16_arm64
 hlu@penguin:~/goldencoin_v1.7.0.2_u16_arm64$ ./goldencoin-qt &
```

### armhf
```
 hlu@penguin:~$ cd  goldencoin_v1.7.0.2_u16_armhf
 hlu@penguin:~/goldencoin_v1.7.0.2_u16_armhf$ ./goldencoin-qt &
```

Above in linux terminal will pop GDNC QT wallet in chromebook desktop.
...
```
 ...
 Then restart the QT GUI wallet, GDNC should start to sync. Note fully synced wallet to latest block is required in order to CPU mine GDNC with Cheetah_Cpuminer
...
```
After you push TAB afer word "nengc" , the chromebook linux terminal should behave like linux in server/desktop with the full file/folder name "goldencoin_v1.7.0.2_u16_arm64" auto populated for you. 
